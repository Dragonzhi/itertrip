...
- 🏨 **酒店比价卡**：价格由用户手动提供（中立，不抓数据；在「✎ 编辑酒店」里直接录入/修改多条报价，或把「🔍 搜索网络报价」的结果一键存入），最低价自动高亮
...
- 🤖 **跑偏了它自己改回来**（M21）：每次生成/改路线都会把每个点与**你要去的目的地**比一遍 —— 与高德所说的位置相差 200km 以上、且改回去离目的地更近，就自动改正并在「🧭 决策过程」里写明「明显偏离用户指定的目的地，已改回」；合法远点（长沙行程里的张家界）不会被同名异地地点搬走，用户手点的位置永不覆盖
- 🗓 **不再撞上闭馆日**（M22）：行程里写着「周一闭馆」这类规则时，会拿**实际日期**算一遍星期 —— 出发日期从「国庆」这类文本就近推断（界面显式标注「推断」，点日历就能改），命中冲突就在时间线标红、并在「🧭 决策过程」里写明；没有日期时如实显示「未检查闭馆日」，**不假装通过**。纯确定性算术，不调模型也不发网络请求
- 💾 **对话不再丢**：首页与规划页的对话（含决策轨迹）刷新后仍在，规划页抽屉可单独清空
- ⏹ **随时能停、断流能看出来**：生成中「发送」变成「停止」（Esc 同效）；服务端每 ~2 秒一次心跳，界面显示「Ns 前有响应」，静默 6s 变金、15s 变红并提示可能已断开、60s 自动中断 —— 没有思考链的模型也能一眼看出是不是还在跑
...
├── DEPLOY.md          # 部署指南（本地/云）
├── docs/              # 里程碑实施记录（M19 坐标可信度 / M20 区域校验 / M21 目的地冲突 / M22 闭馆日检查）
└── LICENSE            # MIT
```
