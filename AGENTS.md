...
| 「说了没做」的两类兜底 | ① **坐标**：prompt 含坐标类措辞而模型 `changed=false` → 后端强制重跑坐标代理真改（§5.4）；② **改路线**（M22.2）：命中改路线意图词但模型**只回叙述、没给 `<<<JSON>>>`** → 自动带纠错重试一次，仍未产出则明说「行程一个字都没变」并把模型的「已设置」标为不算数。两类都不静默、不假装成功 |
