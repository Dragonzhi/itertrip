...
7. **决策轨迹（M19）**：整轮对话逐步下发 `event: trace`——用了哪个模型/从哪来、记忆命中几条（或为什么没注入）、
   ...
   **M24 补充**：坐标来源还有一份**常驻**的摘要，直接长在助手气泡里（折叠轨迹生成完就收起，等于没告诉用户）——
   `coordDigest(reply.route)` 从每个 place/hotel 的 `source`/`confidence` 算出「N 处 · 高德核验 x · AI 推测 y ·
   城市中心 z」与逐条降级点名（`CoordReport`，超 6 条折叠）；闲聊/中断轮没有 route ⇒ 不渲染。
...
| `itertrip:chat` | 对话历史（最近 30 条，剔除 route 快照） | 发请求时仅回传最近 8 条；**同步落盘**（M24：`Chat.tsx` 的 `commit` 通道，不能用 effect —— 生成路线那一轮 AI 回复与「跳规划页」同批提交，Chat 随即被卸载，effect 永远跑不到，AI 整轮消息就是这么丢的） |
...
| 坐标摘要常驻（M24） | 带 route 的回复在气泡内**常驻**一行坐标来源摘要（`coordDigest` 派生自 route 的 `source`/`confidence`）：「N 处 · 高德核验 x · AI 推测 y · 城市中心 z」+ 逐条降级点名（超 6 条折叠）；**判据是可核验的事实，不是新能力**——不新增任何坐标校验，也不改 `coordBadge` 对未知来源不显示徽标的既有行为；闲聊/澄清/中断轮没有 route ⇒ 整块不渲染；`/api/plan` 表单路径无对话消息 ⇒ 不显示（时间线/地图徽标已覆盖） |
...
**非功能性约束**：移动端适配（M23）：<768px 顶栏单行 + 两个侧栏改底部抽屉（互斥、可拖手柄下滑关闭）、日期卡移入抽屉表头、触屏用 ↑/↓ 跨天排序（桌面 HTML5 拖拽保留）、输入框 16px 防 iOS 缩放、`100dvh` + safe-area、Leaflet 下边距补偿；验收脚本 `frontend/scripts/mobile-shot.mjs`（零依赖 CDP，截图 + 溢出/可达性探针 + 拖拽/排序/价格功能断言，支持 `SHOT_URL` 量任意页面）。**包管理器只选一个：本仓已统一为 npm**（`frontend/package-lock.json` 是唯一锁文件；pnpm 的 `pnpm-lock.yaml` 与 `pnpm-workspace.yaml` 已于 2026-09-15 清除）——历史教训：npm 与 pnpm 混装会让 node_modules 出现两份 react（Invalid hook call），切换后先删 `node_modules` 再装；**一键启动脚本的文件编码是硬约束**：`start.cmd` 必须 CRLF、`start.ps1` 必须带 UTF-8 BOM（根目录 `.gitattributes` 已锁 `*.cmd eol=crlf`）——历史教训：LF 的 `.cmd` 会被 cmd.exe 解析成乱码命令（`'v' is not recognized…`）双击直接闪退；无 BOM 的 `.ps1` 会被 `start.cmd` 调用的 Windows PowerShell 5.1 当 ANSI 读，中文字符串里的定界符被吃掉、直接抛 `Unexpected token`。这两种故障 `git status` 一律不提示（git 规范化换行后看起来无差异）、用 pwsh 7 手测也测不出，改完这两个文件请核对首字节与换行；**Tailwind 的 `outline-<宽度>` 只设 outline-width，`outline-style` 的初值是 none**（实测 tailwindcss 3.4.19：产物里 `.focus-visible\:outline-2` 只有 `outline-width:2px`）——单写 `focus-visible:outline-2 focus-visible:outline-moss` 的焦点环**画不出来**；本仓现存 19 处 `focus:outline-2 focus:outline-moss-soft` 是靠同时改 `focus:border-moss` 才侥幸可见，纯文字可聚焦元素（地点名 / 酒店名 / 内联链接）一律用 `index.css` 的 `.focus-ring`，别自己拼 outline 类；
...
- [x] **对话历史不再丢 AI 回复（M24）**：线上现象「首次聊天生成地图路线后，再回对话页只剩用户的消息」。
      根因不在后端、也不在 localStorage 容量：`Chat.tsx` 的落盘挂在 `useEffect` 上，而生成路线那一轮的 AI 回复
      与「跳规划页」是**同一次回调**里的两个 update —— React 18 把它们批处理成一次 render，App 这次直接渲染
      Plan，Chat 在**同一次 commit 里被卸载**，effect 永远跑不到，`itertrip:chat` 就停在「只有 user 消息」的旧快照
      （连下一轮上下文也缺一条）。改为消息只经 `commit` 通道**同步落盘**（`msgsRef` 为唯一真相源，删掉那个 effect），
      顺带修好「流式期间点返回导致 Chat 卸载后终帧被静默丢弃」。变异复现（改回 effect 写法）可见落库序列
      以 `…,"assistant","user"` 结尾 —— 与线上现象逐字一致
- [x] **坐标来源摘要常驻对话消息（M24）**：坐标降级此前只活在折叠的「🧭 决策过程」轨迹里（生成完还会自动收起），
      而坐标恰恰是幻觉最多、用户最难自查的地方。新增 `frontend/src/lib/coordDigest.ts`（从 `reply.route` 的
      `source`/`confidence` 派生「N 处 · 高德核验 x · AI 推测 y · 城市中心 z」+ 可信数与逐条降级点名，
      纯函数零运行时依赖）+ `components/CoordReport.tsx`（气泡内常驻，>6 条折叠，文案与徽标同源），
      挂在首页对话与规划页抽屉两处；**零后端改动、零契约改动**。验证：`node src/lib/coordDigest.check.ts` 16/16、
      `node scripts/chat-stream.mjs` 26/26（新增 [5] 组 9 条断言；`PORT`/`CDP_PORT` 可覆盖，真后端正跑在 8100 时也能并行跑）
