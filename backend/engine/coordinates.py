...
M20 变化（区域校验，见 M20_GEO_REGION_PLAN.md）：
...
def destination_anchor(destination: str) -> tuple[float, float] | None:
    """目的地城市中心（仅限内置城市表内的城市），无则返回 None。

    供上层判断「一个已有坐标是否落在目的地的合理范围内」：现有坐标离锚点过远时，
    说明它本身就离谱，不该再被「行程包络」保护、也不该当同名并列候选的就近裁决参考点。
    """
    _, cities = parse_region(destination)
